...
const getProductDetail = (params) => {
	return new Promise((resolve, reject) => {
		const sql = `SELECT p.id, p.names, p.price, p.img, c."name" as "category_name"
		FROM products p
		JOIN categories c on p.category_id = c.id
		WHERE p.id = $1`;
...
const insertProducts = (data, fileLink) => {
	return new Promise((resolve, reject) => {
		const sql = "insert into product (names, prices, image, categories_id) values ($1, $2, $3, $4) RETURNING *";
...
const updateProduct = (params, data) => {
	return new Promise((resolve, reject) => {
			const sql =
			"UPDATE product SET names = $1, prices = $2, categories_id = $3 WHERE id = $4 RETURNING *";
...
const updateProductWithFile = (params, data, fileLink) => {
	return new Promise((resolve, reject) => {
			const sql =
			"UPDATE product SET names = $1, prices = $2, image = $3, categories_id = $4 WHERE id = $5 RETURNING *";
